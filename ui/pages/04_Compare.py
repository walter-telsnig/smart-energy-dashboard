...
st.set_page_config(
    layout="wide", page_title="Compare • Smart Energy Dashboard", page_icon="📊"
)
...
with left:
    start = st.date_input(
        "Start", value=min_start, min_value=min_start, max_value=max_end
    )
with right:
    end = st.date_input(
        "End",
        value=min(min_start + pd.Timedelta(days=7), max_end),
        min_value=min_start,
        max_value=max_end,
    )
...
overlay, norm, corr, preview = st.tabs(
    ["Overlay", "Normalized", "Correlations", "Preview"]
)
with overlay:
    st.line_chart(
        j.rename(
...
with col3:
    sc = (
        (j[["production_kwh", "consumption_kwh"]].min(axis=1)).sum()
        / j["production_kwh"].sum()
        if j["production_kwh"].sum() > 0
        else 0
    )
